...
function TimeBox({ value, label, compact = false }) {
  return (
    <div className={`rounded-2xl border border-[#173f2f]/10 bg-white text-center shadow-sm ${compact ? 'min-w-[66px] px-2 py-2.5' : 'min-w-[82px] px-3 py-3.5 sm:min-w-[96px]'}`}>
      <strong className={`block tabular-nums text-[#c9963f] ${compact ? 'text-xl' : 'text-2xl sm:text-3xl'}`}>{String(value).padStart(2, '0')}</strong>
      <span className={`mt-1 block font-semibold text-slate-500 ${compact ? 'text-[10px]' : 'text-[11px] sm:text-xs'}`}>{label}</span>
    </div>
  );
}
...
export default function PriceCountdown({ compact = false }) {
  ...
  return (
    <div className={`rounded-3xl border border-[#173f2f]/10 bg-[#f6f1e2] ${compact ? 'p-3.5' : 'p-5 sm:p-6'}`}>
